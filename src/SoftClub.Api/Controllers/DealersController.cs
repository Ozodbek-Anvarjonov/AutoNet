...
public class DealersController(IDealerService service, IMapper mapper) : BaseController
{
    ...
    [HttpPost]
    public async ValueTask<IActionResult> Post([FromBody] DealerDto dto, CancellationToken cancellationToken)
    {
        var data = await service.CreateAsync(mapper.Map<Dealer>(dto), cancellationToken: cancellationToken);

        return Ok(data);
    }

    [HttpPut("{id:int}")]
    public async ValueTask<IActionResult> Put([FromRoute] int id, [FromBody] DealerDto dto, CancellationToken cancellationToken)
    {
        var data = await service.UpdateAsync(id, mapper.Map<Dealer>(dto), cancellationToken: cancellationToken);
